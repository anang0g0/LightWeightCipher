...
時間が経つに連れ、どんどん是弱性が明らかになっていく感じがしますｗ。
...
なぜならカオスは非可逆的だからです。
カオスを利用するような果敢な方法があるかも知れないですが、今のところは使ってないです。
自分自身にXORすることをパイコネと読んでいたつもりでした。
しかしハッシュ関数にパイコネ変換が適用できても、ブロック暗号にこの方法が直接適用できるわけではなさそうです。

今の所、動的にバイト列を入れ替えながらSboxで換字するという方法は復号に成功してます。
...
なので、マスターキーだけでなく置換パターンを秘密鍵にしたほうがより効果的かも知れません。（秘密鍵のサイズが大きくなってしまう）
こうすると、アルゴリズムの一部を非公開にするのと同じようなものなので秘密鍵暗号とは言えない可能性があります。
暗号文だけから置換パターンを推測するのが難しければ、そちらのほうがいいということになります。
