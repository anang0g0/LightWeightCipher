# LowWeightCipher（とりあえず楽そうなテーマで）

# 脳内ダンプ

訂正不可能なエラーを入れた時にできる復号結果は決定不可能だ（別の符号語かもしれない）が、秘密鍵を持っている人だけが暗号文を正しく復号できるというタイプの暗号。

暗号はちょっと難しいみたいなので、認証をやって修行します。

# 20210524

次の目標として、軽量公開鍵暗号の発明に力を入れようと思う。
例としては多変数多項式を使うとかを考えている。

# 20210522

8bitマイコン用の疑似乱数生成器を公開しました。(bit.c)

# 20210515

カオスの本を少し読んだ。
周期の性質や個数について読んだだけで代数や数論との関係に気づいた。（周期：巡回群の位数のようなもの。3周期あるとカオスｰ>判定できる。周期が素数の場合何が言えるか？）

暗号学で言えば数論というのはよく知られているけど、因数分解で言うρ法なんかでも周期点に至るまでの周期に含まれない点列があるということなので、このような数論とカオスも関係がありそうな気がする。
ρ法で2つの大きな素数の合成数からできる周期の大きさは、素因数の平方根に比例しているらしい。
カオスと数論の接点を感じる。

カオスをやる上で、代数学や暗号の知識は活用できるかもしれない。（有限桁である以上、計算機上で真のカオスは実現できない。だから疑似カオス。更に予測困難性を計算量的に再定義できるかどうか？）

私の目指しているものは、計算量的疑似カオスとでも言うべきものなのだろうか？（スネークオイル）

# 20210514

いろいろと充電期間ということで、机に向かって勉強してました。
当分この状態が続くと思います。
今日やったのは、暗号、計算論、位相、論理学、カオスで、そのうち教科書とよべるのは計算論とカオスだけです。
位相なんかもいきなり教科書は無理だと思うので、30講シリーズだったり小嶋本だったりします。
暗号も朝日賞のあの人の読み物ですw

計算論はマイケルシプサの教科書で、半分お話みたいに読めました。
カオスもゆるい入門書です。
数式は出てきます。

ただガリガリと計算するのはつかれるので、読み物中心というかゆるくやってます。
でも読破したらなんとなく必要なことがわかってくるんじゃないかと思います。
微積分にしても計算というよりは、式変形だったり式の意味するものであったりするはずなので、計算とはあまり関係ない気がします。
証明は計算論の本を読みながら身につけたいと思います。
微積分は今日はやりませんでしたがそのうち必要が出てきたらやります。
そんな感じで。

# 20210513

色んな人のアドバイスに従って、暗号理論と符号ベースの暗号の応用に切り替えます。
一応カオスの人にもメールをしたけど、返事が来る可能性は少ないし、返事がなければ縁がなかったとしてやらないことにする。

符号の次は全く別の数理物理や数学理論をやりたかった。
禁止されたわけじゃないけど、やはり自信がないので専門家の言うことを効いていたほうが確実なんじゃないかと思う。

カオス理論という数理物理の領域は以前から乱数との関係があるのではないかと思って興味があった。
今回乱数絡みでカオス理論をやる気になったのだし、そのための準備を教科書の選定から初めて3日くらい時間をかけていた。
証明をマスターするには純粋数学を学んだほうが早いと思ったからだ。
そこに積分論があり、最終的にカオス理論になるという計画だった。
同じようなことばかりではあきる野で、今後符号みたいに1年かかる目標を設定したはずだった。
素人の考えだから出来ない確率のほうが高い、でもやりたい。
そして損をしたくない。
かけた時間を無駄にしたくない。

そうして残ったのが確率論（のはず）だった。

失敗したくないのは山々だけど、確率論を理解するのは自分の能力では到底無理そうな選択肢だとわかる。
理解するどころか使うことさえおぼつかない確率論を、一体どうやって研究すればいいというのか。
カオスをやる背景には、秘密鍵暗号を力学系として扱えるのではないかという予感がしたからだ。
もしカオスをやらないのであれば残る選択肢は圏論だ。
これも前から目をつけていたので、それなりの教科書が揃っている。
一年かけて今までと違うことをしてみたい。
もう二番煎じはしたくない。

~~もともとランダムを数学的に扱うための理論が確率論だったことを考えれば、積分が無駄にならない純粋数学の理論といえば確率論になるだろう。
これなら暗号にも使えるし、測度論までやる必要は感じないけど統計学を応用して実験数学にも使えるだろうし、この選択は捨てていない。
アルゴリズムの話にも確率的アルゴリズムなんて言うのがあるし、確率論が計算とどんな関係にあるのか興味がある。
疑似乱数はやらないほうがいいとみんな口を揃えて言う。
それほど失敗するチャレンジャーが多いのだろう。~~

一応今までどんなことをやったのかそのうち記事にします。
乱数検定テストにはSTS ver3を使いました。
オプションは以下の通り。

sts -v 1 -i 32 -I 1 -w . -F r 

# 20210511

これと言った理論もなしに、置換とS-boxとローテートだけでそこそこ性質のいい乱数が作れるようになった。
NISTのSTSという統計テストを試して、パーフェクトな乱数を生成することも７割位はある。
XORすることで９割位になるけどそこまでは必要がない。
取り敢えず一日中ゲーム廃人のようにガチャを引きまくって、統計テストの成績に一喜一憂する一日であった。
符号をやってたときは毎日課題やバグの修正なんかがあって目標が決まってたけど、乱数の研究？になってから分析力が必要だとわかりこれから理論を始めるところ。
取り敢えず明日からはテスト用の初期値を設定して、途中のデータから置換のパターンを知ることができるかどうか攻撃法を考える予定だ。

プログラミングでは構造体の初期値の設定方法など知らなかったことがわかって嬉しい。

# 20210510

この三日間ずっと乱数にハマってて、専門家にも相手にされず、ずっと一人で考えてきました。
初期値が変わると周期も変わる。
もしかしたらこれはカオスかもしれない。

で、今持っている知識だけではカオス現象を解析するのは難しいので、アルゴリズムからXORをなくして初めからやり直すことにした。

これで少しはわかりやすくなればいいのだが。
純粋に置換と換字だけで乱数ができるのだろうか？


# 20210508

一日中chacha20をいじってました。
すごく簡素な構造をしていてすぐに実装できそうなくらいシンプルでした。
私のはちょっといい加減だったので、quorterround関数を組み込んでみました。
たった一回でぐちゃぐちゃな値が出てくるようになった。

今後はchachaの安全性を分析をする予定。
...
chacha20は正直良くわかりません。  
ちゃんと見てないせいかもしれませんがすごく早いですね。 
この乱数がCSPRNGであるという根拠はどこにあるのでしょう？  
まだまだ勉強が足りません。

# 20210506

https://github.com/shiffthq/chacha20  
暗号学的に安全な疑似乱数というものの実装例chacha20を初めて知ったので、どのようにしてその安全性が確立されているのかを調査中。  
カオスを使ったマイ乱数が出来た。(new.c)  
chacha20を自分のと速度を比較したいけど、テストの書き方がよくわからない。  
自分のは256ビットを10000000回動かして10秒だった。

https://qiita.com/fumumue/items/ad79c9e2f4ed328210f6

# 20210505

午前中は自称カオスハッシュ関数をやっていたが、ループの多さにスピードが遅くなってどうやっても高速化できないので諦めた。
無駄な時間を過ごしてしまった。
こんなことなら最初からAESのS-BOXを使っていればよかった。
午後は符号を使った認証方式をやる予定。

なんだか自分のアイデアってあまり当てにならない。
知らないことが多すぎる。

とりあえずカオス乱数のようなものはできたのだが、これをどうやって評価すれば良いのかさっぱりわからない。
なんだか適当にAESのS-Box使って置換して自分にXORするとかやって、また昔と同じようなことをやってる。
でももう統計データは取らない。
バカでかいデータファイルができるだけでディスクの邪魔者になるからだ。  
周期と言っても64バイト分の空間があるから、512bitくらいの周期はあっても良さそうだ。
これをなるべくわかりやすく説明するための方法を考えなければならない。
この方法が良くないことは同じ関数を使って64bitにしてみればわかることだ。
これを海外のサイトに投稿したら、初期値の選び方によって４０００万以下の周期しかないことがわかっているのだ。
でもそれを理論的に解析する方法がない。
このネタは２０１３年のものだが、有限桁カオスの周期に関する未解決問題だ。

でもS-Boxを使っているとわかるとおり、バイトとビット列では全く性質が違う気がする。（どう違うの？）  
バイナリにしてもいいけど、じゃあS-boxは何に使うの？という感じに。
バイナリにすれば周期などの性質がわかりやすくなると思ったけど、結局使えないと言われて終わりだった。  
どうすれないいのだろう・・・。

こういう未知の問題を分析する能力が研究者の才能というものなんだろうな。

# 20210504

認証なんかやってみようかと思う。
今の所３種類あるけど、それに暗号学的な裏付けをすることが目標。
暗号よりは簡単な気がする。

いい加減に計算しているようで実は意味がある。
で、駄目なら駄目で駄目だという証拠を掴みたい。
安全性の証明のようなことをやってみたい。
暗号はもう当分やらない。

bernstainのsalsa20と手前味噌の当てにならないカオスベースのハッシュ関数を追加ｗ

# 20210115

だめだったわけではないことが判明。
よく考えたらきちんとうまく行った。（lema-scheme）
うまく行ったのは、隣接する頂点重みのXORの総和を表した場合だ。
値が一致しただけなので、本当に巡回セールスマン問題になっているかは確認が必要だ。

# 20210114

2014年にやった自作コミットメント(ovcs.c,logcom.c)をアップ。

詳しい内容は後日説明しますが、自分でも何をどうやったのか覚えてない。
何をやったのかコメントに書いてなかったので、もう一度論文を読まないといけない。
だめなら消します。
...
なんのために新しい概念を定義するかといえば、それを認証に使うためなのだがその認証に使えなければ意味がない。
で、重みの総和が一致すると思ってたら、全部０になっていつも消えるとか、そうでない場合は答えが合わないという感じでだめだとわかった。
応用自体は沢山あるだろうから、それを事前に調べていたら間違いに気づいていたかもしれないし、これから探せば修正できるところがあるかもしれない。
...
ここでやるのはRao-Numの焼き直しであるPACEをもってLWCの代わりとします。

# 20201009

可変式実装とか書いてましたが、アップロードした秘密鍵暗号は符号ベースのもので、既存の秘密鍵暗号の設計手法とは全然違うものです。
SPNとかFeistelに興味を持ったのは、既存の秘密鍵暗号と熱力学の関係を理解しようと思っていたからです。
